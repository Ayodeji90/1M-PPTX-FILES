    ...
    def open_batch(self) -> sqlite3.Row:
        ...
        def _do():
            with self.reg.tx():
                ...
                # Interleaved namespace: node k of N creates batches
                # k+1, k+1+N, ... -- machines can NEVER create the same
                # BATCH folder, so filenames stay contract-exact
                # (BATCH_NN_file_NNNNN.ext) without any node marker.
