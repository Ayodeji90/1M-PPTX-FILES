#!/usr/bin/env python3
"""
CLIP Audit — VM-Local Version

Runs directly on a VM where images are stored locally.
Reads from local disk (fast), classifies with CLIP on CPU,
sorts into PASS/REJECT/BORDERLINE locally, then uploads to Google Drive.

Usage:
    # On a VM with images in ~/1M-PPTX-FILES/data/pages/
    python3 vm_audit.py --input ~/1M-PPTX-FILES/data/pages --output ~/1M-PPTX-FILES/audit_output

    # Then upload to Drive
    rclone copy ~/1M-PPTX-FILES/audit_output gdrive:FIRST/audit/ --progress
"""
...
# ── Lazy imports (install on first run) ──────────────────────────────────

def ensure_deps():
    """Install torch+clip if not available."""
    try:
        import torch
        import clip
        return True
    except ImportError:
        print("Installing dependencies (torch + clip)...")
        import subprocess
...
class CLIPClassifier:
    ...
    def __init__(self, pass_threshold=0.70, reject_threshold=0.40, batch_size=64):
        ...
        self.device = "cpu"  # Force CPU for VM consistency
        ...
        print(f"Ready: {len(GRAPHICS_PROMPTS)} graphics + {len(NON_GRAPHICS_PROMPTS)} non-graphics prompts")
    ...
    def classify_batch(self, image_paths):
        ...
        for i, path in enumerate(valid_paths):
            ...
            if gp > self.pass_threshold:
                classification = "pass"
                reasons = "high_confidence_graphics" if gp > 0.90 else "moderate_confidence_graphics"
            elif gp > self.reject_threshold:
                classification = "borderline"
                if gp > 0.55:
                    reasons = "leaning_graphics_but_uncertain"
                elif gp > 0.45:
                    reasons = "even_split_graphics_vs_non"
                else:
                    reasons = "leaning_non_graphics_but_uncertain"
            else:
                classification = "reject"
                reasons = "clearly_non_graphics" if gp < 0.20 else "likely_non_graphics"

            results.append({
                "filename": os.path.basename(path),
                "graphics_score": round(gp, 4),
                "non_graphics_score": round(np_, 4),
    ...
    def write_csv(file_list, csv_path):
        with open(csv_path, "w", newline="") as f:
            ...
            for r in sorted(file_list, key=lambda x: x.get("graphics_score", 0), reverse=True):
                writer.writerow([
                    r["filename"], r["classification"], r["graphics_score"],
                    r["non_graphics_score"], r["reasons"],
                ])
...
def main():
    parser = argparse.ArgumentParser(description="CLIP Audit — runs locally on VM")
    ...
    print("=" * 60)
    print(f"\n  To upload to Drive:")
    print(f"    rclone copy {output_dir} gdrive:YOUR_FOLDER/ --progress")
